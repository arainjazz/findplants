...
/**
 * 只给**地点文字**做粗化、根本不出示坐标的地方（如「共 N 份」清单里的每一行）用这句。
 * 不能借用 {@link FUZZ_EXPLAIN} —— 那句话讲的是「坐标已模糊」，而那些地方一个数字都不给，
 * 被砍掉精度的是地名本身；照抄会让读者去找一个页面上并不存在的坐标。
 */
export const PLACE_COARSENED_EXPLAIN =
  `该物种命中国家或省级重点保护野生植物名录，为防止盗挖，这里的拍摄地点只显示到` +
  `区/县/旗一级，不显示更精确的位置。`;
