...
export function XiaoPAgentPanel({
  ...
  // 对话框里的第三个视图（另两个是对话与模型设置）。
  // 用户 2026-07-29：「点击小P蛙，把识别结果的缩略摘要卡在对话框里逐条展现」。
  ...
  const send = async () => {
    ...
    try {
      ...
      const rawQ =
        res.imageQueries && res.imageQueries.length
          ? res.imageQueries
          : res.imageQuery
            ? [res.imageQuery]
            : [];
      ...
      setMessages((prev) => [
        ...prev,
        {
          id: uid(),
          role: "agent",
          text: "出错了：" + (e instanceof Error ? e.message : "请稍后再试"),
        },
      ]);
    } finally {
    ...
    <>
      ...
      {!open && !hiddenSide && (
        ...
        >
          ...
          <TaskProgressBars tasks={feed.running} />
      ...
      {open && (
        ...
        >
          ...
          {/* 「对话 / 任务动态」切换条。只在**有动态可看**时出现 ——
              一条任务都没跑过的用户不该被一个空标签占掉一行。
              模型设置视图下整条隐藏（那是个独立的全屏子视图）。 */}
          {!showSettings && feed.rows.length > 0 && (
            <div className="flex shrink-0 border-b border-rule/50 bg-paper-deep/20">
              <button
                onClick={() => setShowFeed(false)}
                className={`flex-1 text-[11px] py-1.5 font-semibold transition-colors cursor-pointer ${
                  showFeed
                    ? "text-ink-faint hover:text-ink"
                    : "text-leaf-deep border-b-2 border-leaf-deep"
                }`}
              >
                对话
              </button>
              <button
                ...
                className={`flex-1 text-[11px] py-1.5 font-semibold transition-colors cursor-pointer inline-flex items-center justify-center gap-1 ${
                  showFeed
                    ? "text-leaf-deep border-b-2 border-leaf-deep"
                    : "text-ink-faint hover:text-ink"
                }`}
                ...
                {messages.length === 0 && (
                  <div className="text-xs text-ink-faint leading-relaxed bg-paper-deep/30 border border-rule/40 rounded-xl p-3">
                    ...
                    {userModel ? (
                      <>
                        当前使用你配置的「
                        <span className="text-leaf-deep font-medium">{userModel.model}</span>
                        」作为我的大脑
                      </>
                    ) : (
                      ...
                      {m.role === "agent" && m.refGroups && m.refGroups.length > 0 && (
                        <div className="mt-2 space-y-2">
                          {dedupGroups(m.refGroups).map((g) => (
                            <div key={g.query}>
                              <p className="text-[11px] font-semibold text-ink-soft italic mb-1">
                                {g.query}
                              </p>
                              {g.loading ? (
                        ...
                        <div className="mt-2 pt-2 border-t border-rule/40">
                          ...
                          {canApply ? (
                            m.applied ? (
                              ...
                              <button
                                onClick={() =>
                                  onImageReplace(m.imageQuery || "", m.editInstruction || "")
                                }
              ...
              >
                ...
                <div className="flex items-center justify-between gap-2 mt-1.5 px-0.5">
                  <span className="text-[10px] text-ink-faint truncate">
                    {userModel ? (
                      <>
                        模型：<span className="text-leaf-deep font-medium">{userModel.model}</span>
                        （我的）
                      </>
                    ) : (
                      <>模型：站点默认</>
