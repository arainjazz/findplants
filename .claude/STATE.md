...
整级丢）、`city` 写成 `city||town||…` 于是**镇被当成市**拼出「内蒙古自治区伊金霍洛镇伊金霍洛旗」
这种倒序、`suburb` 排在 `county||district||suburb` 最后**街道永远轮不到**。新阶梯：
省 → 地级市 → 区/县/旗 → 街道/镇 → 园区/片区 → 村/社区 → 路，每级列一串候选**逐格往下试**
（不能取第一个非空：直辖市 `city`=`state`=「北京市」被去重后，`city_district`=海淀区 必须还能补位）。
三个坑都是**全量 dry-run 逮出来的、不是想出来的**：① `suburb` 与 `town` 是平级两个名字不是备选
（同一点 `town:哈巴格希街道` + `suburb:鄂尔多斯市高新技术产业园区`），挤一级丢了 34 份园区名 →
拆成两级；② 拆开后 21 份冒出**双街道**（哈巴格希街道 + 青春山街道，OSM 边界重叠）→ 加
`SOLE_SUFFIXES=[街道,镇,乡]` 同后缀只留一个，**市/区/县 不能禁**（县级市套地级市合法，会误杀
「通辽市霍林郭勒市」）；③ 地级市**多数地方写在 `city` 里**（`city:衡阳市`+`county:衡阳县`），
只认 `region` 会让 `city` 被区县级吃掉、**衡阳县整级消失** 8 份 → `city` 垫在地级市那级末位。
另加前缀去结巴（`鄂尔多斯市高新技术产业园区` → 去掉已在串里的 `鄂尔多斯市`）。
断言落盘 `scratch/check_place_from_address.mjs`（13 条，全是真实回包/真实回归，**不打网**）：全绿。
🔧 **工具链坑（值得记住）**：Node 的 `fetch`/undici **默认不读 `HTTP(S)_PROXY`**，nominatim 是
境外站 → 241 份齐刷刷 `fetch failed`，而手动 `curl` 明明是通的（curl 读环境变量），极易误判成
被封或被限流。必须 `NODE_USE_ENV_PROXY=1`；脚本已加闸门，检测到配了代理却没开就直接报错并
打印正确命令。Supabase 那半不受影响（`NO_PROXY` 已配直连，EnvHttpProxyAgent 认这个变量）。
回填脚本 `scratch/backfill_capture_place.mjs`：**默认 dry-run**，`--apply` 才写库，`--limit N` 抽样，
1.2s/次守 nominatim 的 1 req/s。全量 dry-run（241 份有坐标）：**改写 209 · 原样 30 · 失败 3**
（3 份是网络抖动，保持原样、可重跑）；变短的只剩 2 份且都是修结巴，双街道 0 残留。
保护物种没被削弱：`coarsenPlace()` 在新格式上实测仍砍回「鄂尔多斯市X区/旗」（街道/村后缀被剥掉）。
tsc=0、build 过、`place-from-address.ts` ESLint 非 prettier 项 0、`identify-plant.functions.ts` 与基线
逐条一致（100 条既有）。
🔴 **最要紧的坦白：这次回填解决不了最初那个问题**。猪毛蒿那 5 行
（39.6441/39.6442/39.6441/39.6445/39.6448，彼此约 80 m）全部落在同一个「哈巴格希街道」，
改完仍是**一模一样一行字**，照样分不开。全站 209 份改写只对应 **26 个不同地名**，其中只有
11 个是某一份独有；最挤的「伊金霍洛镇G210」46 份、「伊金霍洛镇」45 份、「哈巴格希街道高新技术
产业园区」26 份。OSM 在鄂尔多斯的细粒度到此为止。**要真分得开，只有两条路：换国内地理编码
（高德/腾讯有 POI/AOI，需 key）或改用「相对方位+距离」描述（如「同街道内 · 东北约 300 m」）——
等用户定。**
✅ **回填已执行**（用户拍板：跑，但先存旧值快照）。写库前把 213 份旧值落盘成
`scratch/backfill_capture_place.snapshot-2026-08-11T16-53-16.json`（这一列没有版本历史，
覆盖就找不回；脚本新增 `--restore <快照>` 可原样倒回，不打网）。第一次 `--apply` 在**查库那步**
就吃了 `fetch failed` 直接退出（本机代理约一成握手抖动的老毛病，探测 3/3 都 200，是抖动不是断），
补上重试后重跑：**改写 213 · 原样 30 · 反查失败 0**，写库 **210/213**，3 份栽在同一个抖动上，
...
物种来说，把精确坐标塞进 JSON、只在前端决定不画，等于照样公开（页面源码里读得到）；这一栏列的
是别人的观测点，与地图同一处境，判据与脱敏都留服务端。保护物种走 `coarsenPlace()` 砍到
区/县/旗，行尾挂 🔒 + hover 解释（新写 `PLACE_COARSENED_EXPLAIN`，**不能借用 `FUZZ_EXPLAIN`**
——那句讲的是坐标已模糊，这里根本不给坐标）。名录匹配器加了 10 分钟进程内缓存
（`loadConservationMatcherCached`，两千多行要翻三页，而这一栏每开一个页面就判一次），
匹配时补上科名（兰科全科二级，绶草这类只在科一级命中；为此 `plants` 补查 `family`），
名录查不动时**按保护处理**。已验：猪毛蒿 4 行坐标消失、地名照常，**响应体里已无 lat/lng**；
保护那一支把判据临时钉成 true 实测过（→「鄂尔多斯市康巴什区 🔒」），已改回、grep 干净。
tsc=0、build 过、四个文件 ESLint 与基线一致。⚠️ 没找到「真·保护物种且这一栏非空」的样本，
匹配那半段端到端未实测（调用照抄线上跑着的地图那条路）。
🔴 **这一改把地名粒度不齐摆到了明面上**：猪毛蒿 4 行现在全写「内蒙古自治区康巴什区」，
彼此分不开（上一轮靠坐标才分开的）。实测现打 Nominatim：`39.6441,109.8102` →
**哈巴格希街道**, 康巴什区… 比库里存的细一级；且现有 `reverseGeocode()` 把 `suburb` 排在
`county||district||suburb` 最后，街道永远轮不到。**要不要重新反查、怎么落地，等用户定**
（回填 capture_place 全站变细 / 只给这一栏加精细字段 / 保持现状）—— 本轮未动任何数据。
详见文末本轮小节。）_
_上一轮：2026-08-11（续二）— by Claude（**接上被打断的活：同物种「共 N 份」列表每行
...
都出「#阳台杂草」，条目页+特征词出「#路边杂草」。tsc=0、build 过、已提交 `e90772c`。
**🚧 部署仍卡着**：又试一次 `wrangler deploy`，还是 `fetch failed`（今天第 3 次，
详见续四那条 BLOCKER —— 本地代理/TUN 掐上传，curl 打 CF API 却是通的）。
**线上仍是 `80a4e260`，08-09 这四组修复都还没上线**：疑似定名复核 +20、
标签待审草稿重影、返回标签名单乱码。代理关掉后一条命令即可收尾：
`npm run build && ./node_modules/.bin/wrangler deploy`。）_
...
于是永远算待审。真实数据：8 条里 7 条已 approved、只有马唐是真 pending。
改成按归宿分流而不是简单丢弃 —— 有 `published_plant_id` 的记到**它的条目**头上
（采纳那一路条目本就继承 tags，Set 去重；**并入已有条目**那一路目标条目未必挂着这个标签，
丢掉等于这条观测从专题里消失 —— 实测 7 条里正有 3 条属于此类：酢浆草/狗尾草/苏门白酒草）；
approved 却查不到条目的历史脏数据两边都不算。`tags.$slug.tsx` 的**特征词兜底**那条路
...
根因两条：① 解除疑似的判据从前只认 `_editor_diagnosis`，而它是**采纳那一刻**才写的 ——
采纳之前的复核无处记录；② `identification_confidence` 按设计**不能动**（叶子结算读它），
所以标题洗干净了、`isTentative` 照旧判 low。
四改：新增 `ai_payload._xiaop_id_fix` 留痕（小P蛙简介卡改动**动到定名**时写：换了种名/学名，
或指令里就在说疑似/正名/学名）→ 全站解除疑似 + 采纳闸门放行 + 综合可信度 **+20**（写进
评星依据那一行）；判据收进 `lib/tentative.ts` 的 `isDraftTentative`/`tentativeResolution`
（草稿页、待审卡、服务端闸门三处从前各写各的，这次同源）；顺带堵住一个静默副作用 ——
`sanitizeSpeciesName` 会剥「疑似」，于是**任何一次**改摘要都会把标题上的疑似抹平，
现在非定名类改动一律把标记原样补回。tsc=0、改动文件 ESLint 零新增、纯函数离线验过
（45%/5 星 → 65%/7 星，依据末尾多出「编辑已用小P蛙复核定名（名字）、「疑似」已解除 +20」）。
**⚠️ 未部署、未提交；端到端那一趟要编辑账号登录 + 真烧一次小P蛙调用，本轮未验。**
详见文末本轮小节。）_
_上一轮：2026-08-09（续二）— by Claude（**修：「共 2 份」点开只看得到一份**。
用户报拂子茅：绿框「快速简介卡（共 2 份）」、蓝框「银叶科普（共 2 份）」，点开各只有一份。
根因在服务端 `lookupSpeciesExisting`：每一类只 push `group[0]` 一条代表，`count` 报的却是
**全组**数量 —— 数字是真的，第二份**没有任何入口**（银叶连就地展开都只认 `items.find(isInlineItem)`
那一份）。两处改：① 服务端每类**整组**返回（新增 `entries: SpeciesExistingEntry[]`，带
createdAt / capture_place / 缩略图 / 已收录标记；`plants` 没有 capture_place，采纳来的条目
借它**来源草稿**的地点与照片），profiles 查名字从「只查 4 个代表」扩到每一份；
② 前端 `count > 1` 的按钮不再替人选一份，改成**展开成一行一份的列表**（缩略图 + 已收录/
未收录草稿 + 作者 + 日期 + 拍摄地点，四项是用户 2026-08-09 选的），列表里点银叶草稿仍是
**就地展开**、并能在两份之间来回切。**`count == 1` 的行为一个字没动。**
草稿页那个推荐弹窗仍只推最新一份（它的用途就是止住那一枚银叶），副文案改成明说
「先带你看最新的一份 · 其余在下方那一栏点开」。
tsc=0、改动文件 ESLint 零新增（那 6 个 `any` 是既有的，diff 里一个都没加）。
条目页 + 草稿页两处已实测：银叶两份切换时就地展开的 iframe 从 `26e8e741…` 换到 `c96428f8…`；
快速卡两份去向分别是 `/plants/calamagrostis-epigejos-l-roth` 与 `/drafts/63d2601c…`。
**✅ 已部署 Version `80a4e260-ef12-44b7-99fb-6dcfc58fc55a`**（用户要求「连同小P蛙修改一起部署」，
故 08-09 两轮一次上线；两个 chunk 与本地 dist SHA-256 逐字节一致）；**⚠️ 未提交。**
重复本身按用户决定**不动数据**，只改展示。
⚠️ 验证是 DOM 层做的：Browser pane 被另一会话反复收走（viewport 塌成 0×0、滚动锁死），
那一栏的视觉截图没拿到 —— 缩略图只在条目页测到 `naturalWidth=900` 证明能加载，
**行的实际排版没用眼睛看过**。）_
_上一轮：2026-08-09 — by Claude（**修：小P蛙改不动「快速识别简介卡」的正文**。
08-08（续二）修的是**路由**（lite 档不再撞 `</html>` 那道闸），这次修的是**落地**：
① 对话那侧连 `ai_payload` 都没 select，还把不上屏的摘要卡片段当「草稿正文」喂进去，
小P蛙于是开口就是「我在正文里补一段」，`editInstruction` 照这个写；② 字段编辑器只认卡上七项，
收到谈「正文」的指令就判定无处可改、七项原样返回 → `diffDraftCard` 空数组 → 抛那句错。
**关键事实：用户口中卡的「正文」就是卡上的【摘要】**，两处提示词从没说过。
四改：新增共用判据 `isLiteCardDraft`；对话侧 lite 档一律按卡处理并要求指令写成
「把〈某一项〉改成…」；字段编辑器挑明「摘要即正文」+ 第一趟零改动时**追问一次**再报错
（错误文案也改成说清能改什么、不能改的去哪儿改）；范围下拉对 lite 草稿撤掉幻影选项
「整页 · 全文」、默认选中简介卡。tsc=0、build 过、改动文件 ESLint 零新增、
三份真 lite 草稿的下拉已浏览器实测。**⚠️ 未部署未提交；真正「采纳并保存」那一次要编辑账号
登录才能端到端验，本轮未验。** 详见文末本轮小节。）_
_上一轮：2026-08-08（续三）— by Claude（**用户三件事一次做完**：① 分享卡「深度分析
完成后很久才弹」——补掉 08-06 诊断里剩下的三段串行等待（轮询收尾 3s→0.8s；轮询完成那一拍
顺手带回草稿行、喂进缓存，草稿页 loader 不再挡着导航去查一趟库；叶子统计 2s→0.5s、头像
2.5s→0.9s，都改成「先出卡、到了再重画」，重画判据补上 avatarKey）。最坏路径约 8~9.5s → 约 2.2s。
② 「疑似」草稿采纳**必须先写诊断意见**（闸门在服务端，首次收录与并入已有条目两条分支都拦）；
意见落进条目正文 / 草稿正文 / ai_payload / 修改记录四处；条目的标题、摘要、正文结论面上的
「疑似」字样一并消除，但**「识别过程」栏里各模型自评的原话不动**（那是事实留痕）。
刻意不动 identification_confidence——那是叶子结算的输入。③ 标签四步打通：名单页把「在查」
和「确实没有」分开（原来 4 秒空窗里一直写着「此标签下还没有条目」，成因是三表并集跑了两遍）；
名单链接带 ?from=，条目页/草稿页给「← 返回 #标签 名单」；AI 特征词卡签从 404 变成按名字兜底列名单。
tsc=0、build 过、改动文件 ESLint 零新增、新增 15 条离线断言全过、标签那一路浏览器实测。
**✅ 已部署 Version `ffb8a611-0daf-49ba-a933-209b0eed3af6`**（线上 5 个 chunk 逐个核对与本地 dist 一致）；
⚠️ 未提交；识别出卡与编辑弹窗需登录才能端到端验，本轮未验。详见文末本轮小节。）_
_上一轮：2026-08-08 — by Claude（**一修 / 一更正 / 一未做**。
...
全 200；坏 slug 仍 **404**；www 首页 200。草稿页 SSR 的 `<title>`/og:title/og:image
**与改前逐字一致**（分享卡没受影响）。线上实测客户端跳转（chunk 已热）：点击 → 新页
**399ms**、全程没出现 pending；chunk 冷的那一次 1.1s 且如期显示了「载入中…」。
🔎 顺带查清一件**与本轮无关的既有事实**：草稿页的**正文本来就不是 SSR 出来的** ——
改前的线上 HTML 里 `返回 AI 识别` / `待审核草稿` 就是 0 次（正文一直由客户端渲染，
控制台的 React #419 = 服务端那个 Suspense 边界没跑完，同样是既有现象）。本轮唯一的变化是
那块占位从**空白**变成了「载入中…」（`role="status"` 0 → 1），属于改善不是回归。
⚠️ **d（「正在打开…」+ 抽屉等落地才收）仍未在真机上验过** —— 动态流要登录才有卡片。

_上一次：`855aad89-3ce8-4f3f-82ab-40fe2f930b19`，2026-08-13 部署（当天第二次）_
...
**上线核验（已做，这次是用眼睛看的）**：
· 状态码 —— `/` `/identify` `/explore` `/about` `/blog` `/plants/plantago-major` 全 200；
  `/plants/不存在` `/blog/不存在` `/projects/not-a-uuid` **全 404**（部署前全是 200）；
  `/default-og-image.jpg` **200**（部署前 404）。www 域同样：首页 200、坏 slug 404。
· og:image —— 有封面的原样透传 Supabase 绝对地址；无封面的（砂韭）回落
  `https://plantspedia.club/default-og-image.jpg`。
· 视觉 —— `/identify` 与 404 页都截了图，渲染正常，**线上控制台零报错**。
  （注意：STATE 之前记的「浏览器工具对生产域名被策略挡住」这次**没有发生**，能正常看。）
⚠️ **匿名识别的 IP 日限没有在线上跑过计数** —— 验它要么真发一次识别（写真数据、烧真额度），
要么等真实访客触发。逻辑靠 16 条离线断言；线上只确认了 `/identify` 正常打开、无报错。

---

**上一次：线上版本 = `80a4e260-ef12-44b7-99fb-6dcfc58fc55a`，2026-08-09 部署**
（一次带上**两轮**：08-09 小P蛙改不动简介卡正文的四改 + 08-09（续二）同物种「共 N 份」展开列表。
⚠️ **未提交** —— 工作树 15 个文件 1666 行直接打包上线，回滚只能靠 Cloudflare 版本回退。）
**上线核验（已做）**：两个 chunk 拉下来与本地 dist **SHA-256 完全一致** ——
`use-registry-chips-Y3B3HBTD.js`（30556 字节）含 `未收录草稿` / `已收录`；
`drafts._id-BicaYZsq.js`（66898 字节）同时含 `先带你看最新的一份 · 其余在下方那一栏点开`（本轮）
与 `整页 · 全文`（小P蛙那轮），确认两轮都真上去了。
⚠️ 浏览器工具对生产域名被策略检查挡住（javascript_tool / get_page_text 均报
"Policy check temporarily unavailable"），**线上没有用眼睛看过**，核验全靠 curl 对拍字节。

⚠️ **Cloudflare 静态资源有传播延迟**（08-08 记的坑，仍然有效）：部署完 1～2 分钟内拉新 chunk
会拿到 **SPA HTML 外壳**（status 200、content-type 还写着 text/javascript，5131 字节），
看起来像「新代码没上线」。**别据此判定部署失败** —— 等一会儿重取，或直接拿 chunk 的
SHA-256 与本地 dist 对拍（本轮用的就是这招，一次就命中）。

<details><summary>历史（2026-08-08 那一版）</summary>

**线上版本 = `cd2a1f23-c737-4b5a-aace-688bfd203f0e`，2026-08-08 部署**
...
### 已删除三个条目（用户 2026-08-03 决定）
上面那三条「待你定夺」的已发布条目，用户选择**删除条目、保留草稿与照片**。

删前快照：`scratch/deleted-plants-snapshot-2026-08-03120229.json`
（含 plants 3 行完整数据 + plant_edits 6 行 + 3 份回指草稿的状态，可据此还原）。

执行顺序（`scratch/_delete3.mjs`）：
1. 先把 3 份草稿的 `published_plant_id` 摘成 null —— 否则删完会留下指向幽灵的回指；
2. 再 `delete().in("id", ids).select()` —— **`.select()` 不能省**，否则「删了」和「一行没匹配上」
   返回的东西一样（同一天刚在回填脚本上踩过这个坑）；
3. 复查：plants 里 0 行、3 份草稿与 4 张用户实拍照片全部健在。

线上实测：`/plants/cyperus-brevifolius` 等三页均显示「404 · 未收录此条目」。
（curl 看是 200 —— 那是 SSR 骨架，未找到的判定在客户端，别被它骗了。）

**没有级联删的两处，都不是问题：**
- `plant_edits` 里 6 条记录仍指向已删条目 —— UI 本来就处理了这种情况，
  会显示成「（条目已删除）」而不是断链（routes/edits.tsx:652）。留着是诚实的历史记录。
...
## 2026-08-08（续）— 小P蛙换图指令：四步链路已打通（已提交 `6da9bd7` / 已部署 `cd2a1f23`）
...
- ~~未提交、未部署~~ → **写这段时还没提交，后来用户自己提交并上线了**：
  提交 `6da9bd7`、部署 `cd2a1f23`（见本文件顶部「最新线上版本」）。用户在线上实测的结果
  就是下面「续二」那条 bug。

---

## 2026-08-08（续二）— 修：简介卡上「采纳并保存」报「草稿 HTML 不完整，无法自动修改」

用户实测：小P蛙能读图了，但在**快速识别卡**那里点「采纳并保存」必报
`草稿 HTML 不完整，无法自动修改。`

### 根因（实库核过，不是猜的）
「快速识别简介卡」那一档草稿（`ai_payload._enriched === false`）的 `html_content`
是 `buildSummaryCardHtml` 吐的一个**裸 `<div>` 片段**，而 `applyDraftAgentEditFn`
在**进出两头都硬卡 `</html>`**：
- 进：`if (!/<\/html>/i.test(original)) throw new Error("草稿 HTML 不完整，无法自动修改。")`
- 出：`if (!/<\/html>/i.test(html)) throw new Error("小P 生成的内容不完整…")`

线上取样（dev 里连真库查 40 份草稿）：**28 份是 lite（70%）**、12 份是完整文档、
0 份「enriched 但仍是片段」。也就是说这条路对**七成草稿从来就没通过**。
实测那份 `c57b62a5…`：`_enriched=false`、`html_content` 以
`<div style="max-width:680px…` 开头、`</html>` 不存在 —— 与报错完全对得上。

**更要命的一层**：那段片段**根本不上屏**（`drafts.$id.tsx:1876` 是
`notEnriched ? null : <iframe>`），用户看到的全部内容都来自 `plant_drafts` 的**列**。
所以就算把 `</html>` 那道闸放开、让模型去重写片段，也是**烧一次长文改一段谁也看不见的东西**，
而且改完还会和列里的值对不上。

### 三处改动
1. **路由（真正的修法）** `applyDraftAgentEditFn`：select 补上 `ai_payload`；
   `_enriched === false` 时**无论选了什么讨论范围**都走「改字段」那条路
   （原来只有 `scope === DRAFT_CARD_SCOPE` 才走）。自动路由过来却一个字段都没落到时，
   报错文案改成说清「这份草稿目前只有简介卡，要改正文请先点『让 AI 生成进一步介绍草稿』」，
   而不是干巴巴一句「小P 这次没有改动任何字段」。
2. **片段支持（兜底，两条通道都改）**：新增 `isFullHtmlDoc()` / `normalizeRewrittenHtml()`
   放进 `draft-enhance.ts`（该文件零 import，可离线测；本来就是「客户端 + Workers 共用的
   纯字符串 HTML 手术」那一类）。提示词第 3 条按形态切换（完整文档 / 片段），
   收回来时：完整文档仍要求 `</html>`；片段则在模型好心包了一整篇文档时把 `<body>` 里的内容取回。
   `applyPlantAgentEditFn` 同样处理 —— **「采纳快速识别简介」发布出来的条目正文就是同一段片段**，
   线上确有这样的条目（STATE 2026-07-31 记过 3 条），那边原本也是同一个死胡同。
   空正文改为单独的、说得清的错误，不再和「不完整」混为一谈。
3. **不再列幻影范围** `drafts.$id.tsx`：lite 草稿的讨论范围下拉只留「简介卡」，
   不再把那段不上屏片段里的小标题扫出来 —— 选了也只会改到看不见的东西。

### 验证
- `tsc` = 0；`npm run build` 通过；改动范围内 ESLint 零新增。
- 新增离线断言 `scratch/html-shape.test.mjs`（6 条，跑法见文件头）：片段/完整文档判别、
  片段原样收下（**旧代码在这里必抛**）、模型把片段包成整篇文档时取回 `<body>`、
  markdown 围栏、只剩解释文字时如实报错不静默存半截。
- 浏览器实测（dev 5203，连真库）：
  · lite 草稿 `c57b62a5…` → `_enriched=false`、`isFullHtmlDoc=false`（旧闸必拦）、
    范围下拉只剩「整页 · 全文 / 快速识别简介卡」；
  · enriched 草稿 `048e2eae…`（圣诞伽蓝菜）→ 范围下拉仍列全 8 项（拍摄记录/形态特征/
    生境与分布/植物人文/名称溯源/生长条件），**没有回归**；
  · 两页控制台无错误。
- **没验到**：改动后那一次真正的「采纳并保存」没跑过真模型（要登录 + 真烧一次调用）。
  逻辑上这一档现在走的是既有的、一直在用的「改字段」通道，不是新写的路。

---

## 2026-08-08（续三）— 出卡提速三段 + 「疑似」采纳须写诊断意见 + 标签名单的往返

用户三件事，一次做完。**tsc=0、build 通过、改动文件 ESLint 零新增、新增 15 条离线断言全过、
标签那一路浏览器实测（dev 5203，连真库）。✅ 已部署 Version `ffb8a611-0daf-49ba-a933-209b0eed3af6`；⚠️ 未提交。**

> 部署核验（**不要只看 wrangler 说了 Success** —— 见本文件「部署真相」小节）：逐个抓线上 chunk 与本地 `dist/` 比对，
> `tags._slug` / `drafts._id` / `plants._slug` / `draft-card` / `identify` 五个全部命中本轮新增字符串，
> 其中后两个与本地文件**逐字节相同**。deploy 用了 `env -u HTTP_PROXY -u HTTPS_PROXY …`（本机双重代理会掐断上传）。

### ① 分享卡「深度分析完成后很久才弹」——补掉 08-06 诊断里剩下的三段串行等待
08-06 那轮定过根因（本文件 2026-08-06 小节：出卡前串了 5 段等待），当时修掉了 3 段
（叶子统计不再死等、复用本地原图 blob、先等登录态）。**剩下两段没动**，这次连同第三段一起补：

- **轮询 3 秒 → 收尾阶段 0.8 秒**（`lib/poll-job.ts`）：进度 ≥80 之后改用快节奏。
  3 秒是给「模型还在想」那一段用的，而 90% 之后只剩几百毫秒的写库，按 3 秒问等于把
  「其实已经好了」硬压在完成界面上最多再 3 秒。只提速最后一小段，读压力几乎不变。
- **干掉「跳转后再查一次库」那一趟**（`identify-plant.functions.ts` `pollJobFn` /
  `camera-identify.tsx` `goToDraft` / `drafts.$id.tsx` 的 loader）：轮询在**看到完成的那一拍**
  顺手把草稿行一并带回（只在 `status=done && kind=quick_identify` 时带，跑动中包大小不变），
  相机页把它喂进 `["draft", id]` 缓存，草稿页 loader **先看缓存再决定要不要查库**。
  原来这一趟是**挡住导航**的：轮询说完成了，人还盯着「AI 深度分析中」等浏览器往 Supabase
  跑一个来回。喂的是刚从服务端读回来的那一行（比任何本地缓存都新），所以不会重演
  07-30「补拍结果被旧快照顶回去」；`refetchOnMount:"always"` 那道保险原样保留。
- **出卡前的两个上限拆开**（`drafts.$id.tsx`）：登录态仍等 2s（抢跑会出一张没有叶章数的
  访客版卡，差别大），**叶子统计 2s → 0.5s**、**头像 2.5s → 0.9s**。这两样等不到就先出卡，
  到了再重画一次——重画判据里新增了 `avatarKey`（原来只有 stamp + leavesKey，
  所以头像晚到就永远缺一个灰圆，等待砍短会真丢头像）。
- 账面：最坏路径约 3+(0.3~2)+2+2.5 ≈ 8~9.5 秒 → 约 0.8+0+0.5+0.9 ≈ 2.2 秒。
  **没验到**：整条链路要登录 + 真烧一次识别才跑得起来，本轮只做了静态推演 + 手点「生成分享卡」
  这条同源路径的实测（1080×1920 正常出图）。

### ② 「疑似」草稿：编辑必须写诊断意见才能采纳；意见进正文；疑似字样消除
- **闸门在服务端**（`approvePlantDraft`）：判据用全站同源的 `isTentative`（低置信度 /
  摘要开头写疑似 / 名字上标疑似），少于 8 字直接拒。前端弹窗只是提前告知，不是防线。
  **首次收录与并入已有条目两条分支都拦**。
- **意见落三处**：条目正文（新 `applyEditorDiagnosis`）、草稿正文与 `ai_payload._editor_diagnosis`、
  `plant_edits` 的 `draft_approve` 摘要（kind 沿用既有值——那一列的 CHECK 很窄，新值会被静默拒收）。
- **疑似字样消除**（新 `stripTentativeFromHtml`，纯字符串，Workers 无 DOM）：只摘**结论面**
  ——`<title>`/`<h1>`（连「疑似」专用的红色一起）、图片 alt、开头那段摘要，以及 `plants.title`
  与 `plants.summary` 两列。**「识别过程」栏里各模型自评的原话不动**：那是当时发生过的事实，
  改掉就成了伪造留痕；意见块末尾写明「本条目的定种以此意见为准，下方保留 AI 初判记录」。
  另挡一道：「疑似种 / 疑似近似种…」开头的段落不剥（剥了会把话说坏成「种的区分要点」）。
- **不动 `identification_confidence`**：那一列是叶子结算的输入（疑似恒 +1），改它等于顺手
  改了识别人的铜叶数。草稿页改为「有 `_editor_diagnosis` 就不再算疑似」，分享卡/补拍横幅
  同步失效。快速识别档的正文**不上屏**，所以草稿页另用 React 画了一份意见块（识别过程栏
  当年踩过同一个坑）。
- 待审名单（`draft-card.tsx`）新增「疑似 · 待诊断」角标，审稿人点进去之前就知道这条要签字。
- **没验到**：弹窗与角标都在编辑权限后面，本轮没有可登录的编辑账号，UI 只走了类型检查；
  纯函数层有 15 条断言（`scratch/editor-diagnosis.test.mjs`）。

### ③ 标签：点卡签 → 物种名单 → 介绍页 → 回名单，四步打通
- **「此标签下还没有条目」其实是在撒谎**（`tags.$slug.tsx`）：名单要 ~4 秒才出来，
  这期间页面斩钉截铁地说没有条目，读者当场就走。成因是这一页把三张表的分页并集**跑了两遍**
  ——`fetchAllTags` 内部跑一遍算计数，页面又自己跑一遍。改用 `fetchTags` + 一趟 membership，
  并复用 `/explore` 的 `["explore-tag-index"]` 缓存（从地图点进来就是白拿）；
  加了骨架屏，**把「在查」和「确实没有」分开**。条目改成按 id 精确取（原来是拉 268 行整表再过滤）。
- **返回名单**：标签名单里的链接带 `?from=<tagSlug>`，条目页与草稿页据此在页顶给
  「← 返回 #标签 名单」。⚠️ `validateSearch` 只声明 `from` 这一个可选键——写成
  `{...search, from}` 会带上索引签名，TanStack 立刻把 `search` 判成必填，全站二十多处
  `<Link to="/plants/$slug">` 一起类型报错。
- **特征词卡签不再 404**：条目页那排卡签里除了人挂的主题标签，还有 AI 自动填的特征词
  （盐生植物 / 耐盐碱…），它们没有 tags 表的行，链接是拿名字当 slug 拼的，点下去一直撞
  「404 未收录此条目」。现在 slug 查无此行时按**标签名**兜底列名单（`contains(tags,[name])`
  两条精确查询），页头注明这是特征词而非专题。
- 实测：`#圣水草原的植被` 名单 INIT→LOADING→LIST（无假空态）、链接带 from ✓；
  条目页出「← 返回 #圣水草原的植被 名单」✓；`/tags/%E7%9B%90%E7%94%9F%E6%A4%8D%E7%89%A9`
  （盐生植物）从 404 变成正常名单 ✓；草稿页同款返回链接 ✓。

---

## 2026-08-09 — 修：小P蛙改不动「快速识别简介卡」的正文（08-08 续二那条修好路由，没修落地）

用户实测：在快速识别卡上让小P蛙改内容，点「采纳并保存」必报
`这份草稿目前只有「快速识别简介卡」（还没有生成完整正文），而这条指令没有落到卡上的任何一项
（名称 / 学名 / 俗名 / 科属 / 摘要）。要改正文，请先点「让 AI 生成进一步介绍草稿」。`

### 根因（三层，全在「谁知道这份草稿只有一张卡」上）
08-08（续二）把**路由**修对了（`_enriched===false` → 走「改字段」而不是「改 HTML」），
但那之后的三步没人告诉模型实情，于是每次都停在「零改动 → 报错」：

1. **对话那一侧压根不知道这是 lite 档**：`askDraftAgentFn` 的 select 里**没有 `ai_payload`**，
   还把那段不上屏的摘要卡片段当「以下是草稿正文纯文本」喂了进去 —— 小P蛙当然以为有正文，
   `editInstruction` 就写成「在正文中补充…／新增一节…」。
2. **落地那一侧只认卡上七项**，收到一条谈「正文」的指令，对着一份没有 body 字段的 JSON
   判定「无处可改」，七项原样返回；旧提示词还叠了一句「不确定的内容一律保留原值」，
   把这个方向又推了一把。`diffDraftCard` 得到空数组 → 抛那句错。
   **关键事实**：用户口中卡的「正文」= 卡上的【摘要】，旧提示词从没说过这件事。
3. **范围下拉给了个幻影选项**：lite 草稿的「讨论范围」默认是「整页 · 全文」，
   可这一档没有整页；选它等于什么都没选，服务端只能替编辑猜。

### 四处改动
1. `draft-card-fields.ts`（零依赖，客户端/服务端共用）：新增 `isLiteCardDraft(ai_payload)`
   与 `DRAFT_CARD_ITEMS`（七项的中文清单，三处提示词共用一句话）。判据仍是
   `_enriched === false`，与 `notEnriched` / `draftTier` 同源，**不许用 html_content 长度**。
2. `askDraftAgentFn`：select 补 `ai_payload`；lite 档一律按卡处理（喂访客原图 + 卡面快照），
   scope 行改成「这份草稿只有卡、没有正文，摘要就是这张卡的正文」，并要求
   `editInstruction` **必须**写成「把〈某一项〉改成…」；那段片段文字不再叫「正文」，
   改标成「卡渲染出来的文字，不是可编辑的正文」。顺带禁掉这一档的 `imageEdit`/`imagePlan`
   （卡上的图是访客自己拍的，站内搜图换不掉，要改照片得走「补拍」）。
3. `applyDraftAgentEditFn` 的字段编辑器：提示词第 2 条挑明「【摘要】就是这份草稿的正文，
   『改正文/补充/写详细些/换语气』一律落到摘要」；「不确定就保留」收紧到**事实**
   （改写、扩写、精简这类「怎么写」的指令该改就改）。**新增一次追问**：第一趟零改动时，
   把「你原样返回了」摆回对话里再问一次；两趟都不动才报错，错误文案也改成说清
   「能改的是这七项、要换照片/改地点/改标签请走对应入口、要成篇正文请先点银叶生成」。
   ⚠️ 第 2 条与追问语按 `liteCardDraft` 分支：**enriched 草稿上选了卡范围时不能说「摘要是正文」**
   （那份草稿真有正文，会把成篇内容往摘要里灌）。
4. 面板与宿主：`XiaoPAgentPanel` 新增 `defaultScope` / `allScopeLabel`（默认值 = 旧行为，
   其它宿主不受影响）；`drafts.$id.tsx` 对 lite 草稿传 `defaultScope=简介卡`、
   `allScopeLabel={null}` —— 下拉里**不再有**「整页 · 全文」，选项标签也改成
   「快速识别简介卡（本草稿全部内容 · 摘要即正文）」。

### 验证
- `tsc` = 0；`npm run build` 通过；四个改动文件 ESLint 零新增（prettier 已滤，见 memory）。
- 浏览器实测（dev 5203，连真库）：三份 lite 草稿（酢浆草 `3ff68c62`、橙红茑萝 `b623022d`、
  假蒿 `5ec64798`）的范围下拉都只剩一项、`select.value === "快速识别简介卡"`，
  「整页 · 全文」已消失；条目页那只面板照旧不出范围选择器（它本来就没传 scopes），无回归。
- **没验到**：改动后那一次真正的「采纳并保存」仍没跑过真模型 —— 要编辑账号登录 + 真烧一次
  调用。本轮改的是提示词与一次追问，落地通道还是既有那条「改字段」。
  线上这三份 lite 草稿手边没有 enriched 的可对照，enriched + 卡范围那条分支只走了类型检查。
- **⚠️ 未部署、未提交。**

---

## 2026-08-09（续三）— 小P蛙复核过定名的「疑似」：采纳免填意见 + 可信度 +20

### 用户报的现象
草稿「绵毛棘豆 *Oxytropis lanata*」（豆科棘豆属，别名长毛棘豆）已通过小P蛙按
《中国植物物种名录 2026》改用正名，卡面上「疑似」二字早就没有了；点「采纳识别」仍弹
「这份草稿的 AI 结论是「疑似」：请先填写诊断意见（至少 8 字…）」。用户的判断：
**既然已经复核过、疑似已移除，就不该再要一遍意见；而且这一次复核该给判定评分加 20%，
写进评星依据里。**

### 根因（两条，缺一不可）
1. **「解除疑似」从前只有一种记法**：`ai_payload._editor_diagnosis`，而它是
   `approvePlantDraft` 在**采纳那一刻**才写的。采纳**之前**发生的复核（小P蛙改定名）
   没有任何地方记得住 → 闸门无从知道。
2. **`identification_confidence` 按设计不能动**（`lib/leaves.ts` 拿它结算铜叶，疑似恒 +1），
   所以哪怕标题、摘要都洗干净了，`isTentative` 仍然认那个 `"low"` → 判疑似。
   附带一个静默副作用：卡上的中文名要过 `sanitizeSpeciesName`，它会把「疑似」剥掉 ——
   于是**任何一次**改摘要的操作都会顺手把标题上的疑似抹平，页面看不见、闸门照拦。

### 四处改动
1. **`lib/tentative.ts` — 判据同源**（新增，纯函数）
   - `tentativeResolution(aiPayload)` → `{kind:"diagnosis"|"xiaop_fix", at, byName} | null`；
   - `isDraftTentative(aiPayload, {title, summary})` = `isTentative` 三信号 **减去**已签字解除的。
   草稿页（一段内联正则）、待审卡、服务端闸门从前各写各的，加一种解除方式要改三处 ——
   现在只有这一处。
2. **`lib/identify-plant.functions.ts` — 写留痕 + 放行**
   - `applyDraftAgentEditFn` 简介卡分支：判定这一趟算不算**定名复核** ——
     `wasTentative && (剥掉标记后的中文名/学名真的变了 ‖ 指令里在说 疑似|定名|正名|改名|学名|
     中文名|物种名|定种|确诊)`。算 → 摘要开头的「疑似…」一并摘掉，写
     `ai_payload._xiaop_id_fix {instruction, from, to, by, by_name, at}`；
     **不算 → 把 `sanitizeSpeciesName` 剥掉的「疑似」原样补回标题**（与
     `alignMetaToChecklist` 同一手法），别让一次文字润色悄悄改掉这份草稿的定性。
   - `approvePlantDraft`：闸门改用 `isDraftTentative`；`_xiaop_id_fix` 那一路即使没填意见
     也带一份**只洗字样**的 `draftAdoptPatch`（`publishTitle`/`publishSummary` 读的就是它，
     不给的话条目页会留着一个卡面早就不显示的「疑似」）；`draft_approve` 修改记录里写明
     「疑似已由小P蛙定名复核解除（谁 · 哪天）」—— 这是「凭什么免填就采纳了」的唯一凭据。
3. **`lib/identify-trace.ts` — 评分 +20**
   `computeIdentifyConfidence(t, finalSci, final, resolution?)` 多收一个解除留痕：
   `xiaop_fix` 时**不再按疑似压档**，并在末尾加 `XIAOP_FIX_BONUS = 20`，依据行追加
   「编辑已用小P蛙复核定名（名字）、「疑似」已解除 +20」。
   为什么值 20：链路上除 Pl@ntNet 那个分数外全是模型自评，有采纳权的编辑重新定过一遍种
   是唯一的**人类证据**，比二次自动复核（+5，同样是模型）强一个量级。
   ⚠️ **只有 `xiaop_fix` 加分**，`_editor_diagnosis` 那一路一个数都没动（它是采纳时才写的，
   改它会动到已收录条目的星数）。
4. **UI**：草稿页在「编辑诊断意见」那块旁边新增一条「定名已复核 —— 谁 · 哪天 用小P蛙改定了
   本条的定名，AI 初判的「疑似」就此解除（综合可信度 +20…）」。不写出来，页面上就只是
   「疑似两个字凭空消失」，谁在什么时候凭什么定的一概看不出来。

### 验证
- `tsc --noEmit` = 0；改动的 5 个文件 ESLint 在**改动行范围内**零新增（prettier 已滤，见 memory）。
- 纯函数离线跑通（scratchpad 脚本，未入库）：同一份 low 草稿
  复核前 `疑似=true / 45% / 5 星`；复核后 `疑似=false / 65% / 7 星`，
  依据 = 「模型自评疑似档=45；本次无 Pl@ntNet 客观分可参照；编辑已用小P蛙复核定名（谢宗秀）、
  「疑似」已解除 +20」。对照组（只有诊断意见）仍是 45%，未被误加分。
- 浏览器冒烟（dev 5223，连真库）：`/drafts/784e9216…` 渲染正常、可信度栏照常算出 81%/9 星、
  控制台无报错 —— 判据换成共用函数没有回归。
- **没验到**：真正跑一次小P蛙定名改写、再点采纳的端到端那一趟（要编辑账号登录 + 真烧一次
  模型调用）。`_xiaop_id_fix` 目前库里一条都没有。
- **⚠️ 未部署、未提交。** `.claude/launch.json` 多了一个 `dev-alt2`（5223），因为 5203/5213
  都被别的会话占着。

### 已知边界（有意为之）
- 判「是不是定名复核」用的是「名字变了 **或** 指令里在谈定名」。纯润色摘要不会解除疑似，
  也拿不到 +20；反过来，编辑说「去掉疑似」而名字没变**算**解除 —— 那句话本身就是人在拿主意。
- 编辑**手改**简介卡（草稿页那个直接编辑框，走客户端 supabase 写列）不写这条留痕，
  因此不解除疑似、也不加分。要不要一并算，等用户发话。

---

## 2026-08-09（续四）— 修：已收录的条目仍被算成「待审草稿」

### 现象
`#阳台杂草` 页头：「已收录 4 个物种 · 另有 8 条待审草稿」，而下半截「待审草稿（8）」里
禾叶大戟 / 龙葵 / 黄鹌菜 / 纤梗叶下珠 **就是上面刚列过的那 4 个已收录条目** ——
同一株植物在一页上出现两次，第二次还压着一句「尚未收录为正式条目」。

### 根因（一处，波及四个页面）
`lib/tags.ts` 的 `fetchTagMembership` 拉草稿时**只滤 `rejected`**：

```ts
supabase.from("plant_drafts").select("id,tags").neq("status", "rejected")
```

采纳并不删草稿行 —— 它只是把 `status` 置成 `approved` 并写上 `published_plant_id`，
`tags` 原样留在草稿上。于是被采纳的草稿**永远**留在 `draftIds` 里，
计数与名单一路错到底：专题页、`/plants` 的 #tag 归类栏、首页主题浏览器、
`管理 → 主题标签`（`draft_count`）四处同源同错。

真实数据（`阳台杂草`，8 条草稿）：**7 条 `approved` 且有 `published_plant_id`，
只有马唐是 `pending`**；而这 7 个条目里只有 4 个在 `plants.tags` 上带着这个标签
—— 那 4 个就是用户看到的重影。

### 修法：按归宿分流，不是简单丢弃
```
有 published_plant_id → 记到那个条目头上（plants 集合，Set 去重）
approved 但查不到条目 → 两边都不算（历史脏数据，它已经不待审了）
其余                 → 待审草稿
```
**为什么不能只 `.neq("status","approved")` 一滤了事**：另外 3 条（酢浆草 / 狗尾草 /
苏门白酒草）的目标条目**并不带这个标签** —— 它们是「并入已有条目」那条路进来的，
目标条目保留的是自己的 tags。简单丢弃会让这 3 条观测**从专题里彻底消失**（4→4），
而按归宿分流才能把标签接到条目上（4→7）。

`tags.$slug.tsx` 的**特征词兜底** `fetchByTagName` 同一处毛病，同一条规矩
（那条路条目是按 `plants.tags` 反查的，采纳时条目继承了同一批词，摘掉草稿不丢东西）。

### 验证（dev 5223，连真库）
- `#阳台杂草`：**已收录 7 个物种 · 另有 1 条待审草稿（马唐）**，重影消失；
  7 条与库里 7 条 `approved` 逐条对上。
- `/plants` 的 #tag 栏：`#阳台杂草 · 已收录 7 / 共 7 · 待审 1`（原 4 / 待审 8）。
- 特征词 `#路边杂草`：待审 4（疑似旋鳞莎草 / 长刚毛草 / 续断菊 / 一年蓬），
  与库里 4 条 `pending` 完全一致；rejected 的鹅绒藤仍被挡住。
- 地图主题下拉仍为「阳台杂草 · 10 / 圣水草原的植被 · 10」，没有清零 ——
...
## 2026-08-11（续三）— 那一栏不再摆经纬度，改显示「坐标反查出来的地名」（保护物种粗化）

### 用户要的
> 我要你展示的不是坐标，而是坐标背后代表的地点（保护名单里的物种除外）

### 做了什么
- **前端不再出现经纬度数字**，那一行写地名。
- **服务端连坐标都不再回传**。这一条不是洁癖：对保护物种来说，把精确坐标塞进 JSON、
  只在前端决定不画出来，等于照样公开（页面源码里读得到）。这一栏列的是**别人**的观测点，
  与地图那条路（geo-sightings.functions.ts）同一处境 —— 判据与脱敏都留在服务端。
  查询里的 `capture_lat/capture_lng` 一并撤掉。
- **保护名单例外**：命中「国家/省级重点保护野生植物名录」的物种，地点由服务端
  `coarsenPlace()` 砍到区/县/旗，砍不出来写「地点已隐去」，行尾挂 🔒、hover 出解释。
  身份一律不看（不像草稿页给站长/资深编辑/本人放行）——一份紧凑清单不值得为此多跑一趟鉴权，
  分享卡那条路同样是「命中名录就一律模糊」。
- 名录匹配器加**进程内缓存** `loadConservationMatcherCached()`（geo-sightings.functions.ts
  导出，10 分钟 TTL，失败的那次不留）：`conservation_taxa` 两千多行要翻三页，而这一栏
  每次打开条目页/草稿页都要判一次。顺手让地图那条路也享受到。
- 匹配时**科名一定要给**：兰科全科是国家二级，绶草这类只在科一级命中。草稿行没有就退到
  条目行 —— 为此 `plants` 的 select 补了 `family`（原来只有草稿查了 family，
  纯条目的一栏会把科级保护整个漏掉）。
- 名录查不动时**按保护处理**（宁可少给信息，也不能把一株四合木的位置漏出去）。
- 新增 `PLACE_COARSENED_EXPLAIN`（protected-coords.ts）：**不能借用 `FUZZ_EXPLAIN`** ——
  那句讲的是「坐标已模糊」，而这里一个数字都不给，被砍精度的是地名本身，照抄会让读者
  去找一个页面上并不存在的坐标。

### 验证（dev 5203 连真库、未登录）
- 猪毛蒿（非保护）4 行：坐标消失，地名照常；**服务端响应里已无 `lat`/`lng` 字段**
  （逐字读了 `_serverFn` 的响应体，只有 `place` + `placeCoarsened:false`）。
- 保护那一支：把判据临时钉成 true 实测过一次 —— 四行的
  「内蒙古自治区康巴什区」变成「鄂尔多斯市康巴什区 🔒」，hover 出新那句解释；**已改回**
  （`grep TEMP-VERIFY` 干净）。
- tsc=0、build 过、四个文件 ESLint 非 prettier 问题与基线一致（links 4、functions 6，
  geo-sightings / protected-coords 各 0）。
- ⚠️ **没跑到一个「真·保护物种且这一栏里有内容」的页面**：裸果木、绶草都命中名录，
  但它们那一栏本身是空的（同物种没有第二份内容），库里一时找不到符合条件的样本。
  匹配那半段是照抄地图已在线上跑的调用（`match(sci, family)` + `protectedLists.size>0`），
  但**端到端没实测**。

### ⚠️ 遗留：地名粒度不齐，这一改把问题摆到了明面上
猪毛蒿那 4 行现在**全写「内蒙古自治区康巴什区」**，彼此再也分不开 —— 上一轮正是靠坐标
（39.6441 / 39.6445 / 39.6448）才区分开的。根子在 `capture_place`：它是识别当时
`reverseGeocode()` 写下的，全库 251 份里 44 份只到区一级。
实测拿其中一个坐标现打 Nominatim：`39.6441,109.8102` → **哈巴格希街道, 康巴什区,
鄂尔多斯市, 内蒙古自治区** —— 比库里存的那条细一级。两个原因：① 当时那次反查就只拿到区；
② 现有 `reverseGeocode()` 拼的是 `province+city+county+road`，`suburb`（街道）在
`county||district||suburb` 里被前两个挡住，永远轮不到。
**待用户定**：要不要用坐标重新反查一遍把地名做细（回填 `capture_place` 全站变细 /
只给这一栏加一个精细字段 / 保持现状）。未做任何数据改动。

---

## 2026-08-12 · 高德 key 到位后的地名重做（**代码已改，库未动**）

### key
`.env` 已有 `AMAP_KEY`（Web服务类型，自检脚本实打实调通）。**线上还没配** ——
Worker 读不到 `.env`，要 `wrangler secret put AMAP_KEY` 才在 plantspedia.club 生效。

### 结论先行：那 5 份猪毛蒿，换高德也分不开
`scratch/check_amap_key.mjs` 拿真实坐标实测：5 个点（彼此约 80 m）高德全部回同一个
AOI「和悦云锦」。**靠地名区分这 5 份是做不到的**，不是接口选型问题，是那儿确实
没有更细的地物。要分开只剩「相对方位+距离」这条路，尚未做。

### 换高德真正买到的东西
1. **纠错**。OSM 在这一带是坏的，两处实测（`scratch/probe_dingbian.mjs` 用镇中心距离判）：
   - 羊柴 37.5627,107.8219 → 砖井镇 5.9 km vs 白泥井镇 18.4 km，**OSM 错**；
   - 裂叶滨藜 37.8811,107.8147 → OSM 给「内蒙古自治区…**定边县**」，省县互斥，本身是坏数据。
2. **真地标**上墙：成吉思汗陵旅游区、圣水草原、华侨城国家湿地公园、各高速服务区。

### 两处自己拿的主意（`place-from-amap.ts` 的 `isPublishable`，要改就改这一个函数）
1. **住宅一律不上墙**。高德在居民区直接给小区名（和悦云锦 `120302`）甚至楼栋号
   （「和悦云锦12栋」）。这一栏是公开的，访客多在自家附近拍 —— 等于替人公布住址。
   保护名录那套粗化只管**物种**，管不到这件事（泄露的是人不是植物）。
   光按类型筛不够：还得把住宅实体的**名字**拉黑，否则「和悦云锦接待中心」(生活服务)、
   「百姓饭店(悦和城店)」(餐饮) 照样把小区抖出来。
2. **商户不算地点**（白名单 `PLACE_LIKE_TOP`）。只筛住宅的话兜底会挑到最近的商户：
   「·隆升源美味牛肉拉面(珠江店)」「·海川口腔」「·中共鄂托克旗委巡察工作领导小组办公室」。
   只留地物：景区/公园/湿地/园区/高速服务区/校园/村庄名/道路名。匹配不上退回街道级。

### 现状
- 断言 14 条全过（`scratch/test_place_from_amap.mjs`，用的是真实响应片段）；tsc=0。
- dry-run：**改写 196 / 原样 47 / 失败 0**，全库只会出现 15 种「·地物」写法。
  住宅名残留 grep 为 0。
- **库里现在还是上一轮 OSM 回填的值**，高德这版一个字没写进去。
- 未部署（累计 5 个 commit 未上线）。

### 已写库（用户 08-12 拍板「就按这版」）
两轮 `--apply`：先 196 份，复验揪出 1 份漏网（`假蒿 · …龙城工业园安全文明小区办公室`
—— 办公室归在「地名地址信息」里，类型合规但它是机构不是地点，还捎带「小区」字样），
补了 `isOrgNotPlace` + 排除「培训机构」后再写 2 份。

**复验（`scratch/verify_capture_place.mjs` 直接查库，不看日志）**：
243 份有坐标 · 住宅类词 **0** · 商户类词 **0** · 空地名 **0** · 共 31 种写法 ·
带「·地物」后缀 95 份。断言 17 条全过，tsc=0。
旧值快照两份都在 `scratch/backfill_capture_place.snapshot-*.json`，可 `--restore`。

**浏览器实看（dev 5203 连真库）**：猪毛蒿页
- 识别人行 → 「…伊金霍洛镇·成吉思汗陵旅游区」「…·圣水草原」✅
- 那 4 份未收录草稿 → 仍全是「…康巴什区哈巴格希街道」，靠**时间+可信度**区分
...
🔧 **上限可以在后台改，不用重新部署**（用户追问「撞上 60 怎么办」之后补的）：
往 `site_config` 写一行 **`anon_identify_daily_limit`**，值直接写数字，下一次请求就生效；
删掉这行回到代码里的默认 60。同 `ai_model_config` 的路子。**写 `0` = 彻底关掉匿名识别**，
这是被刷爆时的急停开关，不用等一次部署。
计数行和这个覆盖值**一次查询取回**（`.in([计数key, 上限key])`），没给每次识别多加往返。
`parseLimitOverride` 容忍几种手填写法（`150` / `"150"` / `{"limit":150}` / `{"value":150}`），
但负数 / 小数 / 非数字 / 空对象一律回 null 退到默认值 —— 手滑填个 `-1` 不该把游客识别关死。
断言：`scratch/check_identify_rate_limit.mjs`，**31 条全绿**，不打网、不碰库。
...
### 2026-08-15（续二）— 已部署 `e1ee5176`，线上核验完毕

`5d4275f` 已上线（见文首「部署真相」）。核验里值得记住的两条：

1. **线上提速有数**：客户端跳转到草稿页，chunk 热的情况下点击 → 新页 **399ms**（改前基线
   1.1–1.4s），且**没有**出现 pending 占位；chunk 冷的第一次是 1.1s 并如期亮了「载入中…」——
   这正是 b 该出现的场合，不是 bug。touchstart 预取实测比点击早 534ms（c 生效）。
2. 🔎 **一个查清了的既有事实，别再当成本轮回归**：草稿页的**正文从来就不是 SSR 出来的**。
   把部署前后的线上 HTML 摆在一起数：`返回 AI 识别` / `待审核草稿` 两边都是 **0 次**，
   `疑似龙蒿` 两边都是 13 次（全在 `<head>` 的 meta 里）。也就是说服务端那个 Suspense 边界
   一直没跑完（控制台的 **React #419** 就是它的客户端症状，`/blog` 这类页面则一个错都没有）。
   本轮唯一的变化是那块占位从**空白**变成了「载入中…」（`role="status"` 0 → 1）。
   ⚠️ 判定「是不是我改坏的」时**要拿部署前抓的 HTML 逐 token 数**，别只看控制台有没有红字 ——
   这次差点把一个既有现象当成自己的回归。（想真修它是另一件事：得查为什么 drafts 那条
   Suspense 边界在 Workers 上没被 flush 完，本轮没动。）

### 下一步
- 请用户在**手机上真点一次**小P蛙 → 任务动态 → 任意一张卡片，确认 d 的观感：
  「正在打开…」→ 页面换掉 → 抽屉才收起。这是唯一没能在本机验的一环。
- 若仍觉得慢，下一个可动的点是那 15 个路由 chunk（drafts._id 67KB + use-registry-chips 31KB），
  以及 SW 对 `/assets/*` 的 stale-while-revalidate 开销（实测每个 chunk 即使命中缓存也要 120–165ms）。
